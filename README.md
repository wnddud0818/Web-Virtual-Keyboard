...
USB 장치 클래스는 단일 HID 인터페이스가 종류를 결정하도록 설정하며, CDC·MSC·DFU 자동 시작과 WebUSB는 명시적으로 꺼 둡니다. 키 입력의 전송 성공 여부를 검사하는 HID 리포트도 이 키보드 인터페이스를 그대로 사용합니다. 운영체제가 같은 키보드를 HID 항목과 키보드 항목 양쪽에 표시하는 것은 별도의 기능이 추가된 것이 아닙니다. BOOT 버튼을 누르고 연결하는 ROM 다운로드 모드는 이 애플리케이션의 키보드 전용 설정이 적용되지 않습니다.
...
- **청크 단위 파일 타이핑** - 자동, 원문 텍스트, 파일 복원/Base64 중에서 모드를 고르고 전송 바이트 기준 청크 크기와 키 간격을 설정한 뒤, 전송 전 예상 소요 시간과 브라우저 진행률을 확인합니다.
- **자동 압축 파일·텍스트 전송** - 파일 전체를 gzip으로 압축한 뒤 실제 입력량이 줄어들 때만 압축본을 보냅니다. 한글을 비롯한 모든 UTF-8 텍스트도 지원하며, 디코더가 자동으로 압축을 풀고 원본을 검증합니다. 실제로 타이핑되는 것은 Base64뿐이므로 디코더가 원본 문자를 정확히 복원합니다. [ASCII 외 텍스트 보내기](#ascii-외-텍스트-보내기)를 참고하세요.
...
- **프리셋** - 자주 쓰는 문구를 저장하고 클릭 한 번으로 보냅니다. 값은 US-ASCII와 탭·줄바꿈만 지원하며, 한글 등은 저장·전송 시 거부됩니다. 이름과 그룹에는 한글을 사용할 수 있습니다. 이전 펌웨어에서 저장한 ASCII 외 값도 타이핑하기 전에 검사합니다.
...
1. 키보드 레이아웃을 **영어(미국)**로 선택하거나, Windows 한국어 Microsoft IME를 **영문(A)** 상태로 둡니다.
2. **Caps Lock이 꺼져 있는지** 확인합니다. 타이핑되는 WVK1/WVK2 스트림과 Base64 페이로드는 대소문자를 구분합니다.
3. 원문 모드라면 비어 있는 일반 텍스트 편집기를, 파일 복원 모드라면 `decoder.html`의 데이터 입력창을 포커스합니다.
...
### Windows가 한글 입력 상태일 때

받는 PC에서 전송받을 입력창을 포커스하고, 웹 화면 상단의 **Windows 한/영 전환** 버튼을 한 번 누르세요. 대상 입력기가 `가`에서 `A`로 바뀐 것을 확인한 다음 텍스트·프리셋·파일·디코더 소스를 전송합니다. 전송 후 한글로 돌아가려면 같은 버튼을 다시 누릅니다.

이 버튼은 Windows 한국어 키보드 **101키 종류 1**의 오른쪽 Alt를 보냅니다. [Microsoft 한국어 IME 설명](https://learn.microsoft.com/ko-kr/globalization/input/korean-ime)의 한/영 전환 방식이며, 다른 키보드 종류나 사용자 지정 단축키에서는 직접 영문 상태로 전환해야 합니다. 현재 입력기 상태를 읽거나 영문으로 고정하는 기능은 아니므로 이미 `A`이면 누르지 마세요. 통신 오류가 나면 실제 키는 전달되었을 수 있으므로 PC 상태부터 확인합니다. 진행 중인 전송에서는 전환 요청을 거부합니다. 한글 원문 전송은 기존 **텍스트를 디코더로 전송**을 사용하세요.

## 파일 전송 모드

### 자동

파일을 선택하면 브라우저가 파일 전체를 gzip으로 압축하고, Base64 패딩·청크 레코드·헤더까지 포함한 실제 타이핑 문자 수를 비교합니다. 원본보다 작아지고 기존 전송보다 입력량이 줄어들 때만 **압축 파일 복원(WVK2)**을 선택합니다. 화면에 압축 전후 크기, 입력량 감소율, 받는 PC에서 포커스할 입력창, 예상 소요 시간이 표시됩니다.

압축 효과가 없거나 브라우저에서 압축을 지원하지 않으면 기존 선택 방식으로 돌아갑니다. 출력 가능한 US-ASCII와 탭/CR/LF만 있는 파일은 **원문 텍스트**, 나머지는 **WVK1/Base64**입니다. 압축된 ASCII 파일도 디코더에서 복원해야 하므로 전송 전에 표시된 입력창 안내를 확인하세요. **원문 텍스트**를 직접 선택하면 항상 압축 없이 편집기에 타이핑합니다.
...
이 모드는 항상 디코더로 복원하며 텍스트 파일과 바이너리 파일 모두에 쓸 수 있습니다. 자동 압축을 적용할 수 없으면 기존 WVK1을 사용합니다. 브라우저가 원본 바이트를 청크로 나누고, 각 청크를 Base64로 인코딩하고, 원본 청크마다 CRC32를 계산하고, 원본 파일 전체에 대한 SHA-256을 계산합니다. 동글은 다음과 같은 스트림을 타이핑합니다.
...
압축은 보내는 브라우저에서, 압축 해제는 받는 디코더에서 수행합니다. 동글은 압축된 청크만 타이핑하므로 파일 전체를 저장하거나 압축을 풀지 않습니다. gzip 데이터 전체를 만든 뒤 청크로 나누어 청크마다 압축하는 방식보다 중복 패턴을 더 잘 활용합니다. `CompressionStream("gzip")`과 `DecompressionStream("gzip")`을 사용하며 외부 스크립트나 인터넷 연결이 필요하지 않습니다.

```text
WVK2
NAME=example.txt
SIZE=<gzip 바이트 수>
CHUNKS=<압축된 데이터의 청크 수>
SHA256=<gzip 전체의 SHA-256>
ENCODING=gzip
ORIGINAL_SIZE=<원본 바이트 수>
ORIGINAL_SHA256=<원본 전체의 SHA-256>

C|000001|<압축 청크 CRC32>|<압축 청크 Base64>
...
END
```

수신 디코더는 청크 순서·개수·CRC32, 압축 데이터 크기·SHA-256을 검증한 뒤 gzip을 풀고, 원본 크기·SHA-256까지 검증합니다. 해제 중 원본 크기를 초과하면 즉시 중단합니다. 모든 검사를 통과한 경우에만 원래 파일 이름으로 저장하거나 UTF-8 텍스트를 표시합니다. 압축된 바이트와 검증값은 재시도 시 그대로 재사용하며, 준비 중 중지를 누르면 장치로 전송하지 않습니다.

**업데이트 시 받는 PC의 `decoder.html`도 한 번 교체해야 합니다.** 최신 디코더는 기존 WVK1과 일반 Base64도 지원합니다. 예전 디코더는 WVK2를 복원할 수 없으며, 압축 해제를 지원하지 않는 브라우저에는 최신 Chrome을 사용하라는 오류가 표시됩니다. 기존 파일을 덮어쓰기 전 새 디코더 소스를 빈 편집기에 타이핑해 저장하거나 장치에서 내려받으세요.

## ASCII 외 텍스트 보내기

USB HID는 문자가 아니라 키코드를 전달하고, HID 키코드 표에는 한글이나 그 밖의 ASCII 외 문자에 해당하는 항목이 없습니다. 그래서 자유 텍스트 입력창과 원문 모드는 US-ASCII만 받습니다.

WVK1/WVK2는 문자 자체를 타이핑하지 않으므로 이 문제를 완전히 우회합니다.

1. 브라우저가 텍스트를 UTF-8 바이트로 인코딩하고 자동 압축을 확인한 뒤 Base64로 변환합니다.
2. 동글은 순수 US-ASCII인 Base64 봉투만 타이핑합니다.
3. `decoder.html`이 CRC32와 SHA-256을 검증하고, 압축된 경우 압축을 풀어 원본 바이트를 복원·검증합니다.

이 방식으로 텍스트를 보내려면 전송 카드의 **텍스트 보내기** 입력창에 붙여 넣고 **텍스트를 디코더로 전송**을 누르세요. 전송 모드 설정과 무관하게 자동 압축이 적용되며, 압축하면 WVK2, 그 밖에는 WVK1로 전송되고, `message.txt`로 도착합니다.
...
5. 제어용 브라우저로 돌아가 파일을 선택하고 **파일 복원 / Base64 (자동 압축)** 모드로 전송합니다.
...
- 브라우저 기본 설정은 청크당 전송 데이터 768바이트(압축 시 gzip 바이트 기준)이며 허용 범위는 96-1536바이트입니다. 동글로 전송되는 Base64 페이로드는 2048자로 제한됩니다. 키 간격은 0-100 ms 범위에서 조절할 수 있고 기본값은 5 ms입니다.
- 브라우저가 전송 전에 고유 ID를 만들므로, 시작 응답이 유실되어도 같은 ID로 시작을 재시도해 기존 세션을 복구합니다. 이 기능은 현재 부팅에서 장치가 보관 중인 세션에 적용됩니다. 진행 중 브라우저를 새로고침하면 세션 정보를 잃으므로 장치를 재부팅하고 대상 내용을 비운 뒤 새로 시작하세요.
- 브라우저는 다음 청크를 보내기 전에 장치의 0부터 시작하는 `next` 카운터를 기다립니다. 이미 받아들인 청크를 다시 보내는 것은 멱등하므로, HTTP 응답이 불확실해도 같은 청크가 두 번 타이핑되지 않습니다. **재시도 / 이어서**는 멈춘 브라우저-동글 세션을 복구합니다.
- **중지**는 장치의 취소 응답을 받아야 완료로 표시합니다. 응답을 받지 못하면 세션과 중지 버튼을 유지하므로 연결 복구 후 다시 중지할 수 있습니다. 시작 응답이 오기 전에도 중지할 수 있습니다.
- USB가 준비되지 않았거나 키 누름·해제 리포트 전송이 실패하면 오류로 중단하고 해당 청크를 완료로 집계하지 않습니다. 실패한 키 입력은 자동으로 재전송하지 않으며, USB 복구 시 눌린 키를 해제합니다. 일부 입력은 이미 전달되었을 수 있으므로 대상 내용을 비우고 새로 전송하세요.
- 진행률은 동글이 HID 키 리포트를 내보냈다는 뜻입니다. 키보드 프로토콜에는 대상 편집기로부터의 문자 단위 응답이 없으므로, 포커스된 애플리케이션이 모든 문자를 실제로 받았는지는 증명할 수 없습니다. WVK1/WVK2의 CRC/SHA 검증은 그 뒤에 대상 컴퓨터에서 이루어집니다.
- 자동 압축이 적용되면 입력량이 줄어든 만큼 타이핑 시간도 대체로 줄어듭니다. ZIP·JPG·MP4처럼 이미 압축된 파일은 효과가 작을 수 있습니다. 압축하지 않은 경우 Base64는 타이핑할 문자를 약 33% 늘립니다. 대략적인 하한선으로, 100 KiB는 키당 2 ms에서 약 4.6분, 5 ms에서 약 11.4분이 걸리고, 1 MiB는 각각 약 47분과 117분이 걸립니다. HTTP와 애플리케이션 오버헤드가 여기에 더해집니다. 청크를 크게 하면 요청 오버헤드는 줄지만 키 입력 횟수는 줄지 않습니다.
- 파일을 선택하거나 텍스트를 입력하면 **예상 소요 시간**이 함께 표시되고, 전송 모드·청크 크기·키 간격을 바꾸면 즉시 다시 계산합니다. 실제로 타이핑될 문자 수(봉투 헤더와 Base64 패딩 포함)에 키 간격, 문자당 HID 리포트 2회(약 2 ms), 청크마다 HTTP 왕복과 상태 폴링(약 250 ms)을 더한 근삿값입니다. 자동 압축이 적용되면 압축 후 기준으로 계산하며, 텍스트 입력창의 값은 압축 전 기준이므로 실제 전송은 그보다 짧아질 수 있습니다. 무선 상태와 대상 PC에 따라 달라집니다.
...
| `POST` | `/transfer/start` | 원문, WVK1 또는 gzip WVK2 세션 시작. WVK2는 `sha256`, `originalSize`, `originalSha256`이 필수이며 `size`는 gzip 바이트 수입니다. 선택적인 32자리 16진수 `id`로 동일 세션의 시작 재시도 지원 |
...
- USB 키 입력을 받을 대상 컴퓨터
- WVK1/WVK2 디코딩을 위한 대상 컴퓨터의 Chrome 또는 최신 브라우저
...
저장소 루트에서 실행합니다. 네이티브 테스트에는 C++17 컴파일러와 PlatformIO가 설치한 ArduinoJson 의존성이 필요합니다.

```sh
node Web-Virtual-Keyboard-platformio/tests/index_script_test.mjs
node Web-Virtual-Keyboard-platformio/tests/decoder_protocol_test.mjs
node Web-Virtual-Keyboard-platformio/tests/compression_transfer_test.mjs
node Web-Virtual-Keyboard-platformio/tests/transfer_recovery_test.mjs
python3 Web-Virtual-Keyboard-platformio/tests/native_test.py
```

브라우저 테스트는 시작 응답 유실, 재시도, 취소 실패, 요청 시간 초과를 검증합니다. 압축 테스트는 Node.js 22 이상에서 한글·바이너리 왕복 복원, 압축 효과 비교, 미지원 브라우저 대체 경로, 손상된 데이터·원본 검증값 거부, 압축 중 취소를 확인합니다. 네이티브 테스트는 실제 HTTP 핸들러와 HID 엔진을 가짜 USB·서버·저장소에 연결해 프리셋 검증, 중복 시작, 취소 순서, USB 오류 시 완료 카운터 보존과 WVK2 헤더·청크의 실제 키 리포트를 확인합니다. 실제 보드의 USB 절전·복구 동작은 별도 하드웨어 검증이 필요합니다.
...
- 펌웨어는 미국식 키보드 매핑을 전제합니다. 유니코드 텍스트는 직접 타이핑하지 말고 파일 복원 모드로 전송해 `decoder.html`에서 복원해야 합니다.
