...
2. **Caps Lock이 꺼져 있는지** 확인합니다. 타이핑되는 WVK1 스트림과 Base64 페이로드는 대소문자를 구분합니다.
...
| `POST` | `/transfer/start` | 원문 또는 WVK1 세션 시작. 선택적인 32자리 16진수 `id`로 동일 세션의 시작 재시도 지원 |
...
브라우저 테스트는 시작 응답 유실, 재시도, 취소 실패, 요청 시간 초과를 검증합니다. 네이티브 테스트는 실제 HTTP 핸들러와 HID 엔진을 가짜 USB·서버·저장소에 연결해 프리셋 검증, 중복 시작, 취소 순서, USB 오류 시 완료 카운터 보존을 확인합니다. 실제 보드의 USB 절전·복구 동작은 별도 하드웨어 검증이 필요합니다.
