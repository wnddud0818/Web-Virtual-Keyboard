...
- **청크 단위 파일 타이핑** - 자동, 원문 텍스트, 파일 복원/Base64 중에서 모드를 고르고 전송 바이트 기준 청크 크기와 키 간격을 설정한 뒤, 브라우저에서 진행률을 확인합니다.
...
파일을 선택하면 브라우저가 파일 전체를 gzip으로 압축하고, Base64 패딩·청크 레코드·헤더까지 포함한 실제 타이핑 문자 수를 비교합니다. 원본보다 작아지고 기존 전송보다 입력량이 줄어들 때만 **압축 파일 복원(WVK2)**을 선택합니다. 화면에 압축 전후 크기, 입력량 감소율, 받는 PC에서 포커스할 입력창이 표시됩니다.
